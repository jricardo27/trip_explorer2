...
app.put("/api/trips/:tripId/days/:dayId", async (req, res) => {
  ...
  try {
    ...
    console.log("Updating trip day:", { dayId, tripId, body: req.body, query: queryText, values })
...
app.put("/api/trip-days/:dayId/features/:id", async (req, res) => {
  ...
  try {
    ...
    console.log("Updating feature:", { id, dayId, body: req.body, query: queryText, values })

    const result = await query(queryText, values)

    console.log("Update feature result:", result.rows[0])
