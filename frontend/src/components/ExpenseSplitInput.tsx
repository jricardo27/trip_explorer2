...
export const ExpenseSplitInput: React.FC<ExpenseSplitInputProps> = ({
    ...
    const recalculateSplits = (
        ...
        const activeMembers = currentSplits.length > 0
                            ...
                            secondaryAction={
                                isIncluded && splitType !== "equal" ? (
                                    <TextField
                                        ...
                                        type="number"
                                        value={
                                            splitType === "percentage" ? split.percentage ?? 0 :
                                                splitType === "shares" ? split.shares ?? 1 :
                                                    split.amount ?? 0
                                        }
            ...
            {splitType !== "equal" && (
                <Box sx={{ display: 'flex', justifyContent: 'space-between', px: 2, mt: 1 }}>
                    ...
                    <Typography variant="caption" color={
                        Math.abs(value.reduce((a, b) => a + b.amount, 0) - totalAmount) > 0.1 ? "error" : "success.main"
                    }>
                        {currency}{value.reduce((a, b) => a + b.amount, 0).toFixed(2)} / {currency}{totalAmount.toFixed(2)}
                    </Typography>
                </Box>
            )}
        </Box>
